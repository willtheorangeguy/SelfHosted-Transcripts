...
• Inboard as a current solution for managing bookmarks and links
• Comparison of Inboard to self-hosted bookmark manager Service
• Dog sheep: a personal analytics tool that collects data from various cloud services and stores it in a SQLite database
• Linde: a cloud hosting platform offering $100 for 60 days on new accounts, with one-click applications and object storage capabilities
• Peer Tube: a self-hosted video platform used by the host, deployed using Nix OS and Docker
• AlmaLinux and other distributions available on Linde's marketplace
• Object storage benefits and use of Linde's S3 compatible storage for backing services like Nextcloud
• The CI (Continuous Integration) works when manually building, but a webhook issue was previously broken.
• A new feature in Gift version 1.16 introduced a webhook allowed host list variable that caused the issue.
...
• Discussion of Google strategy and self-hosting infrastructure alternatives (e.g. Peer tube)
• Review of the impact on businesses (e.g. sponsor Tail scale) due to Google's decision
• Early adoption of technology by smart individuals can lead to trickle-down effects in the enterprise.
• Self-hosted solutions like Fast mail may benefit from people using them at home, which then leads to adoption in work environments.
...
• Tail Scale is a VPN service that allows users to connect multiple devices securely and easily, with features such as zero-config and real-time network switching.
