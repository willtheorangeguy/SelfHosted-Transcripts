...
• Guest shares their Black Friday deals, including a FlexiSpot sit-stand desk and SSDs for a RAID Z1 array
...
• Comparison between Intel X710 DA2 networking card and Mellanox cards
...
• Discussion on Intel's tick-tock model and minor upgrades between generations
• Hardware transcoding performance in Jellifin tested and compared to other GPUs
...
• Setting up Talescale for a flat mesh network
• Zero-configuration VPN built on WireGuard
• Simplifying networking and reducing connectivity issues
• Talescale's features and benefits for self-hosting and ad hoc networking
...
• They are asked about keeping raw footage for their YouTube channel and admit to storing it all, including ProRes files, which takes up more space.
...
• Comparison of different motherboard manufacturers (ASRock, ASUS, MSI)
...
• NetGate situation and the confusion surrounding the story
• NetGate is the commercial entity behind PFSense, a firewall distribution built on FreeBSD.
• PFSense has different tiers of licensing, including free Community Edition (CE) and paid Plus option.
• The CE license was previously available for free, but was later revoked without proper communication from NetGate.
• Users were left with only one option to pay $399 per year for TAC support contract.
• After backlash, NetGate introduced a new TAC Lite subscription for individuals at a lower cost of $129 per year.
...
• NetGate's actions and motivations
• History of NetGate's conflicts with the PFSense community, including:
	+ The "OpenSense" website controversy in 2017
...
• Similar themes of code theft and free-riding in other companies (e.g. Prusa, HashiCorp)
• NetGate's attempt to sponsor WireGuard implementation in the BSD kernel and subsequent issues
• NetGate and PFSense controversy due to allegedly rushing code that wasn't properly audited
• Scott Long's blog posts about PFSense Plus and dev insights were found to be "economical with the truth"
• AES-NI flip-flop incident where NetGate initially required CPUs to have it, then later reversed course
• WireGuard situation with FreeBSD, where NetGate's implementation was insecure and they refused to acknowledge the mistake
• Patterns of behavior in NetGate's actions that raise trust issues, including prioritizing profits over open-source principles
• Transition to Linode as a hosting provider, now part of Akamai
...
• New product announcement: 45homelab.com premium storage servers for HomeLab
...
• Handling MDNS traffic to traverse VLANs and solve connectivity issues
