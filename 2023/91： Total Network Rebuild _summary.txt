...
• Discussion of custom firewall software and OpenSense
...
• Discussion of UniFi switches and their features, including PoE power and auto-negotiation capabilities
...
• OpenSense as a VLAN orchestrator and potential performance considerations
• Consideration of using a Layer 3 switch for hardware-based acceleration
• Unify SwitchFlex minis as small, managed switches with PoE support
...
• Using VLANs (Virtual Local Area Networks) to tag ports and create separate networks
• Configuring OpenSense box to use T-Mobile connection as a backup
• Load balancing between T-Mobile and Spectrum connections
• Using UniFi switches and SwitchFlex minis to manage network settings
• Troubleshooting internet connectivity issues with easy switch-over between VLANs
• UniFi cloud controller allows creating dedicated VLANs for Wi-Fi SSIDs
• Routing internet traffic over VLANs and setting up separate networks for specific devices
• Linode cloud hosting discussed, including one-click deployment options and features like Jitsi and NextCloud
• Benefits of using Linode, such as reasonable pricing, data centers around the world, and ability to disperse infrastructure
...
• Using Smokeping to ping switches and identify packet loss
...
• Replacement of old Wi-Fi gear with new UniFi equipment, including U6 Pros and 12 new switches
• Introduction to Netbox as an alternative to DHCP and IPAM system
• Discussion about being sick while working on networking projects and the challenges of troubleshooting network issues
• Upgrading a CPU from a Celeron G4900 to an i5-9500 in an HP290 Slim case
...
• Discussion of Smokeping and other services
• Comparison of Odroid H3+ performance with Pi 4
• Plans to build an Odroid K3S cluster
...
• A user commented on a blog post about an EC-C22 device exposing the IPMI web GUI to the internet when rebooting.
• The commenter, Badco, described how he fixed the issue by moving the WAN port to another Ethernet port and applying a fix.
• Alex explained that Rock Rack BMC implementations have a dedicated IPMI LAN port and two gigabit ports for connecting devices to the LAN.
• When multiple IP addresses are assigned to these ports, it can lead to issues with VLANs and IPMI functionality.
• A user described how they had to use an IPMI raw command to turn off the BMC on one of the NICs to resolve the issue.
• Update on Audiobookshelf iOS client
• Issues with UI and Scrobble bar resolved in new update
• New layout moves playback controls above Scrobble bar
...
• Feedback from listeners praising Audiobookshelf
• Discussion of Intel Arc GPU and its ease of use in Linux
• Intel Arc technology and its potential for virtual machines
• IGPU pass-through and GVTG ( Graphics Virtualization Technology) challenges
• Creation of an IGPU benchmark specifically for media encoding purposes
...
• Introduction to JB Jobs room for job seekers and employers to connect
• Self-hosting updates, including link in show notes at self-hosted.show/slash/91
• Thank yous for boosts received from listeners, including Sam Squanch's generous donation after an outage
...
• Comparison of Audible to Libro FM, an independent audiobook platform
