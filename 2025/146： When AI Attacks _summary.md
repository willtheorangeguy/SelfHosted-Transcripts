...
• Concerns about Medibuntu (MB) being closed source and how it compares to Jellyfin
• Discussion of AI bot scrapers impacting open source projects, including Wikipedia's recent article on the topic
• Introduction of guest Z Yasso, who has written extensively on the subject of AI bots and their impact on self-hosted infrastructure
...
• The speaker has implemented a "proof of work" system to block AI scrapers, inspired by an email spam prevention method called hashcash
...
• Concerns about inadvertently locking out browsers that don't support JavaScript or Big Endian systems
• Exploring alternative solutions, including WebAssembly and Equa X
...
• The tool is most tested with Gitforges and may have varying success rates depending on the specific use case
...
• Unraid 7.1 has been released with new features including wireless networking support and improved ZFS pool importing
• The guest discusses their home lab setup, which is based on Unraid and runs Kubernetes and Longhorn.
...
• The guest defends using Kubernetes in a home lab, acknowledging its complexity but arguing it's worth the learning opportunity for developers who want to retain skills like site reliability engineering (SRE).
...
• Alternatives to Kubernetes mentioned: Doku (like Heroku but with Docker), Docker Swarm
...
• Discussion of storage options, including Longhorn and ZFS with RAID Zed 2 (RAID 5 with hotspare)
...
• Apollo app and its open-source implementation of NVIDIA GameStream
...
• Upgrades to the narrator's server setup with a new 9950X based compute node and i5 9500 media server
• Muso's hardware setup in an apartment with multiple rooms providing services and need for UPS solutions
...
• Recap of boost statistics and appreciation for SREs and audience members supporting the show
...
• LinuxFest Northwest approaching
