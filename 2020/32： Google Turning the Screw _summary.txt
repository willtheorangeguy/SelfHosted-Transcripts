...
• BookSonic software for hosting and playing self-hosted audiobooks with features like last playback position and chapter support
• Integrating BookSonic into Plex media server for a low-hanging but imperfect solution
...
• Benefits and advantages of using Linode over other cloud providers
...
• Options for migrating data, including manual migration or using XCal client
• Next Cloud CLI client used for moving servers
• Using a Linode instance for faster performance and redundancy
• Security considerations: exposing the system outside of firewall, potential for port scanning and attacks
• Recommendations for securing the Linode instance and using its built-in firewall features
• Benefits of having an off-site backup system with Next Cloud on Linode
• Data security advantages of employing off-site backups to a cloud service
• Difficulty of self-hosting an email server
• Hybrid approach to self-hosting emails using a service like MailRoute or Amazon SES
• Unix mindset in building email systems
• Advantages of having a receiving email server while farming out sending portion to a relay service like Mailgun or Amazon SES
...
• Intermediary services like MailRoutes and Postini (now owned by Google) for sending and receiving email
...
• Chris mentions using TASMOTA devices and MQTT for home automation
...
• Example of using multiple DNS services in conjunction with each other (e.g. OpenSense, AdGuard Home)
• Considerations for running Kubernetes and OpenShift clusters on a LAN
• The host is currently using PyHole for GHCP server and dynamic DNS
...
• Using PhotoPRISM for photo organization and backup, including auto-backup from phone to file server
• Potential future blog post or segment on combining PhotoPRISM with other open-source software for a comprehensive photo workflow
• Discussion of the author's experiences with Docker and its packaging issues on Linux distributions
• Mention of considering switching to Podman due to Docker's semantic versioning changes
...
• Social media presence of the hosts (Twitter: ironicbadger, Chris LAS, self-hosted show)