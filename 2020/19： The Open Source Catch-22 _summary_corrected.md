...
• The host mentions a "red flag" but notes that the developer has dialled back some changes, indicating they are listening to feedback
...
• The host discusses a new project called Home Lab OS, which promises easy deployment and management of personal data centres
• The host notes that Home Lab OS uses Terraform under the hood and creates a Bastion server for secure remote access
• The project is complex and abstracted, making it difficult to understand what's happening under the hood.
• It uses Ginger emulating for Docker Compose YAML files and supports hundreds of applications.
...
• Home Lab OS discussed as a project with potential if made more generic and reusable
...
• Discussion of using pfSense as a firewall and OpenVPN for streaming player due to IP block requirements
• Description of a complex process for installing Ubuntu 2004 on an old server
• Discussion of remote access to ESXi console through IP KVM
...
• Using Ganoid tool to manage snapshots and replication with ZFS send/receive functionality
...
• Comparison with other file systems (XFS, Butters) and their checksumming capabilities
• Discussion of Snap Raid as an alternative to ZFS for checksumming
...
• Linking health checks in show notes