...
• Thermal issues with server gear while travelling
...
• NASA devices are easy to integrate with Home Assistant
• AV-1000 Power line adapters were used for networking in an RV
• They provided 300 megabits transfer rate between devices
• POE gear and power distribution was also discussed
• The user wants more information on using multiple Power line adapters
...
• LED light strip installation in a cupboard with temperature-controlled colour change
• Automation setup using Pep Wave and bonded VPN for cellular routing
...
• Promotion of Linde cloud hosting provider and its features
• The user tested different distributions (e.g. WordPress on top of various web servers) to find the best one for their needs.
• They used Linde's $100 credit to deploy multiple instances and test each distribution.
• The user also reviewed Linde, stating that it has been around forever and knows how to do its job.
• The discussion then shifted to power usage, specifically testing the Helios, an ARM-based CPU.
• The user compared the Helios' power draw to other systems (i5-8500 and Dual Leon) under idle and full load conditions.
• The results showed the Helios using significantly less power than the other two systems.
• Power consumption of different systems (Helios, i5, Dual Leon) with various loads
...
• System performance differences between Helios, i5, and Dual Leon processors
• The i5 CPU has quick sync and uses less energy than the Leon
...
• The speaker considers consolidating their two Raspberry Pis with Proxmox on a ThinkPad with Quick Sync.
• Energy savings: using a single system with Quick Sync instead of multiple Raspberry Pis results in significant energy savings (35 watts vs. 300 watts).
...
• The KVM project enables remote access to the system using a BMC Immigrate interface.
...
• The conference used a platform called Hoping that facilitated networking and meeting new people
• Home Assistant announced its first hardware device, "Home Assistant Blue"
• The speaker compares this device to the Droid N2 Plus and Raspberry Pi 4, discussing their features and advantages
...
• Improved non-Google voices via Nebulas Home Assistant Cloud
• Temporary device disabling feature
• New feature to disable and re-enable devices remotely
• Discussion on the Waze 3 camera, a $20 Wi-Fi camera with no monthly subscription required for basic features
• Comparison of Waze 2 and Waze 3 cameras, including design improvements and new features such as a wider field of view and improved night performance
