• The hosts discuss their Prime Day purchases, including a Fire tablet for $50 and a SodaStream.
...
• Data centers and hard drive engineering
...
• Comparison of shucked vs unshucked drives and their performance
...
• Promotion of Linode hosting service and its Terraform support
...
• Recommendation to use Linode for cloud hosting with a $100 credit offer
...
• Question from audience member Optimus Gray about database management, specifically whether to merge multiple MariaDB instances into one or keep them separate
...
• An email from Jay is read aloud, describing an issue with NFS remounting on FreeNAS after waking up from sleep.
• The speaker suggests using SystemD to auto-remount file systems when the network connection comes back online.
...
• Challenges of developing software for large ARM systems in data centers
• VMware's strategy to provide a development environment for ARM code that can be deployed in the cloud
• Potential use cases, such as sales meetings or traveling professionals needing a portable virtual environment
• Announcement of a pick, Archivey, a self-hosted knowledge repository for preserving and organizing useful content.
